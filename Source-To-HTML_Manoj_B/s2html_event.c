...
pevent_t *pstate_idle_handler(FILE *fd, int ch)
{
	...
	case '\'':				// begining of ASCII char
		...
		else //		LINE LITERAL IS BEGGINING
		{
	...
	case 'a' ... 'z':		// could be reserved key word
		...
		{
#ifdef DEBUG
			printf("alpahbits scanning Begin : /*\n");
#endif
...
pevent_t *pstate_sub_preprocessor_main_handler(FILE *fd, int ch)
{
	...
	static int space = 1; //will act lke a marker to identify each word and let the event take care of the word after it finishes
	switch (ch)
	{
	case ' ': 
		space = 0;
	...
	case '"': // checks for user header
		...
		return &pevent_data;
		break;
	case '\n': // if the is only preprocessor ex.#endif
		set_parser_event(PSTATE_IDLE, PEVENT_PREPROCESSOR_DIRECTIVE);
		pevent_data.property = USER_HEADER_FILE;
		return &pevent_data;
		break;
	case 'a' ... 'z': // if is is a macro
	case 'A' ... 'Z':
		if (space)
		{
			pevent_data.data[event_data_idx++] = ch;
...
pevent_t *pstate_reserve_keyword_handler(FILE *fd, int ch)
{
	...
	else
	{
		...
		{
			if (strcmp(pevent_data.data, res_kwords_data[i]) == 0)
			{
				fseek(fd, -1, SEEK_CUR);
				pevent_data.property = RES_KEYWORD_DATA; //imporant to set the property as well
				set_parser_event(PSTATE_IDLE, PEVENT_RESERVE_KEYWORD);
				return &pevent_data;
			}
		}
		for (int i = 0; i < res_kwords_non_data[i][0] != '\0'; i++)
		{
			if (strcmp(pevent_data.data, res_kwords_non_data[i]) == 0)
			{
				fseek(fd, -1, SEEK_CUR);
...
pevent_t *pstate_numeric_constant_handler(FILE *fd, int ch)
{
	...
	case '.': //also need to include the decimal na 
		pevent_data.data[event_data_idx++] = ch;
		break;
	default: // collect the number 
#ifdef DEBUG
		printf("\nnumeric end\n");
#endif
		fseek(fd, -1, SEEK_CUR);
...
pevent_t *pstate_ascii_char_handler(FILE *fd, int ch)
{
	...
	for (int i = 0; i < SIZE_OF_SYMBOLS; i++)
	{
		...
		if (ch == symbols[i])
		{
			...
			postch = fgetc(fd);
			pevent_data.data[event_data_idx++] = ch;
			pevent_data.data[event_data_idx++] = postch;
	...
	if (ch == '\\')
	{
		char postch, postpostch;
		postch = fgetc(fd);
		postpostch = fgetc(fd);
		pevent_data.data[event_data_idx++] = ch;
		pevent_data.data[event_data_idx++] = postch;
		pevent_data.data[event_data_idx++] = postpostch;
		set_parser_event(PSTATE_IDLE, PEVENT_ASCII_CHAR);
		return &pevent_data;
	}
